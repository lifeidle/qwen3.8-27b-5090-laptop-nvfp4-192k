...
| **150K + `--ctx-checkpoints 4`** | **86.2 tok/s** | **6.1 s** | ✅ **推荐（甜蜜点）** |
| 148K | 80.4 tok/s | — | ✅ |

> 生成速度与上下文呈非线性关系（显存贴边时静默进入极慢回退模式——不是 OOM）。**细扫发现 150K 是峰值**：152K 起快速滑坡（63.6），150K 处 86.2（≈纯文本水平）。视觉模式推荐 **150K**；纯文本模式仍可用 192K。
